import type { Metadata } from "next";
import { Geist, Geist_Mono } from "next/font/google";
import "./globals.css";
import { EntityProvider } from "@/lib/entity-context";

const geistSans = Geist({
  variable: "--font-geist-sans",
  subsets: ["latin"],
});

const geistMono = Geist_Mono({
  variable: "--font-geist-mono",
  subsets: ["latin"],
});

export const metadata: Metadata = {
  title: "Avaken Finance OS",
  description:
    "Unified personal & company finance operating system for Avaken — TikTok Shop affiliate revenue, VAT, tax and wealth in one premium dashboard.",
};

export default function RootLayout({
  children,
}: Readonly<{ children: React.ReactNode }>) {
  return (
    <html lang="en-GB" className={`${geistSans.variable} ${geistMono.variable} antialiased`}>
      <body>
        <EntityProvider>{children}</EntityProvider>
      </body>
    </html>
  );
}
